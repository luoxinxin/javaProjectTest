package com.luoxinxin.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.luoxinxin.domain.SysLogininfor;
import com.luoxinxin.service.SysLogininforService;
import com.luoxinxin.mapper.SysLogininforMapper;
import org.springframework.stereotype.Service;

/**
* @author xinsilinwang
* @description 针对表【sys_logininfor(系统访问记录)】的数据库操作Service实现
* @createDate 2022-01-20 09:34:55
*/
@Service
public class SysLogininforServiceImpl extends ServiceImpl<SysLogininforMapper, SysLogininfor>
    implements SysLogininforService{

}




